...
namespace ExamineActionsAPI
{
    public class ExamineActionState
	{
        ...
        public bool CheckCanPerformSelectedAction (IExamineAction act)
        {
            ...
            if (act.ShouldConsumeOnSuccess(this) && act.GetConsumingUnits(this) > (Subject.m_StackableItem?.m_Units ?? 1)
             || act.GetConsumingPowderKgs(this) > (this.Subject.m_PowderItem?.m_Weight.ToQuantity(1f) ?? 0f)
             || act.GetConsumingLiquidLiters(this) > (this.Subject.m_LiquidItem?.m_Liquid.ToQuantity(1f) ?? 0f))
            {
                return false;
            }
        ...
        internal void GetAllMaterials(out List<MaterialOrProductItemConf>? items, out List<MaterialOrProductLiquidConf>? liquids, out List<MaterialOrProductPowderConf>? powders)
        {
            ...
            {
                liquids = new();
                eapl.GetMaterialLiquid(this, liquids);
            }
            powders = null;
